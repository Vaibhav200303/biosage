...
def assess(profile: EnvironmentalProfile, *, query: str = "", retriever: EvidenceRetriever | None = None) -> AssessmentResponse:
    ...
    preserve_primary_crop = bool(re.search(r"(?:cannot|can't|can not)\s+(?:stop|replace|remove)|(?:must|need to)\s+(?:keep|continue)\b|keep\s+(?:(?:growing|planting)\s+\w+|the\s+\w+|\w+)|continue\s+(?:growing|planting|with)\s+\w+", query.lower()))
    for rule in active_rules:
        ...
        if rule.key == "diversify_crops" and preserve_primary_crop:
            action = "Keep the primary crop and pilot a compatible intercrop or rotation window"
